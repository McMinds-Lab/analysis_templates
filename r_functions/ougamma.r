rougamma <- function(n,shape,scale,time,sigma2r_mean,start) {
  ...
  ## integration of sigma^2 over a timeperiod can be modeled as the sum of many small deviations, of a number proportional to time, each drawn from a gamma distribution
  ...
  ## note that in this model, the expectation for total evolution (sigma2r_t) increases proportionally with time, but so does the precision of our estimate
  ## small time periods have more variable and more skewed possibilities, with more 'outliers'
  ## as the base shape goes to infinity, variability of evolutionary rate goes to 0, and the model converges to standard OU evolution with fixed sigma
  ...
  ## the deterministic portion of OU evolution (when the attractor is 0) can be calculated as a simple fraction of the starting value, with the fraction defined as:
