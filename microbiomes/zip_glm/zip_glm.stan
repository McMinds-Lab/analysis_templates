functions {
    ...
    } // probably much faster to separate zeros and non-zeros in transformed data, then vectorize nonzeros separately
}
data {
...
transformed data {
    ...
    for(f in 1:K_f) idxk_f2[NB_f+f-1] = NFB + f;
}
parameters {
    real<lower=0> global_scale_prevalence;
    real<lower=0> global_scale_abundance;
    vector<lower=0>[(NSB+K_s+1)*(NFB+K_f+1)-2] sd_prevalence_norm;
    vector<lower=0>[(NSB+K_s+1)*(NFB+K_f)-1] sd_abundance_norm;
...
transformed parameters {
    matrix<lower=0>[NSB+K_s+1,NFB+K_f+1] sd_prevalence = to_matrix(append_row(100, append_row(sd_prevalence_norm, 1.0)), NSB+K_s+1, NFB+K_f+1) * (prior_scale_p * global_scale_prevalence);
    matrix<lower=0>[NSB+K_s+1,NFB+K_f] sd_abundance = to_matrix(append_row(sd_abundance_norm, 1.0), NSB+K_s+1, NFB+K_f) * (prior_scale_a * global_scale_abundance);
}
model {
    ...
    matrix[NS,NF] prevalence
        = XL_s * (beta_prevalence_i .* sd_prevalence[idxk_s,idxk_f]) * XL_f
          + XL_s * (beta_prevalence_s .* sd_prevalence[idxk_s, rep_array(NFB+K_f+1,NF)])
          + (beta_prevalence_f .* sd_prevalence[rep_array(NSB+K_s+1,NS), idxk_f]) * XL_f;
    matrix[NS,NF] abundance
        = XL_s * (beta_abundance_i .* sd_abundance[idxk_s,idxk_f2]) * XL_f[2:,]
          + XL_s * (beta_abundance_s .* sd_abundance[idxk_s, rep_array(NFB+K_f,NF)])
          + (beta_abundance_f .* sd_abundance[rep_array(NSB+K_s+1,NS), idxk_f2]) * XL_f[2:,]
          + residuals * sd_abundance[NSB+K_s+1, NFB+K_f]
