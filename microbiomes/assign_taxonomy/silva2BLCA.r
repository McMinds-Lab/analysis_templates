...
tax <- read.table('silva2BLCA/SILVA_138.1_SSURef_tax_silva_clean_accession_taxonomy.txt', sep='\t',quote='')
...
write.table(tax, file='silva2BLCA/SILVA_138.1_SSURef_tax_silva_clean_accession_taxonomy_BLCA.txt', row.names=FALSE, col.names=FALSE, sep='\t', quote=FALSE)

dna <- Biostrings::DNAStringSet(gsub('U','T',Biostrings::readBStringSet('silva2BLCA/SILVA_138.1_SSURef_tax_silva_clean.fasta')))
dna <- dna[tax[,1]]
Biostrings::writeXStringSet(dna,'silva2BLCA/SILVA_138.1_SSURef_tax_silva_clean_euks_BLCA.fasta.gz',compress=TRUE,format='fasta',width=10000)
