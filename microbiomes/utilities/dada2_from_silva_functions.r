...
makeTaxonomyFasta_SilvaNR_18S <- function (fin, ftax, fout, include.species = FALSE, compress = TRUE) {
  ...
  taxa.ba.mat[grep(paste(c("uncultured",'unidentified','artificial','fungal','endophyte','eukaryote','metagenome','environmental','Incertae','-like','_gen.','_nr.','_flagellate'),collapse='|'), taxa.ba.mat, ignore.case=TRUE)] <- NA
  set.seed(100)
  N_EUK <- 100
